...
--  syntax ρ ∣ b > x = ρ b (return x) -- where open Monad truncMonad
  ...
  record _⇔[_]_ (A : SubEl)  (j : Carrier) (B : SubEl) : Set (e ⊔ o ⊔ lsuc lzero) where
    ...
    field
      A⇒B : A ⇒ B
      B=>'A : F B =>[ UNC (proj₁ ( A⇒B)) ] F A
      lenProof : (UNC (proj₁ A⇒B)) ≤ j

  0⇔ker : (p : k ≤ j) (ed : j ≤ i) → 𝟎 j ⇔[ i ] Ker p
  0⇔ker p ed = (proj₁ (0⇒ker p)) §$ ((intro⊆ (p , λ (x , _) → *)) , re) §$ ed
  0⇔ker' : (p : k ≤ j) → 𝟎 j ⇔[ j ] Ker p
  ...
  int↔ ed p = int⊂ (proj₁ p) §$  IncUncert' refl=>' reflexivity , reflexivity  §$ ed
  ...
  infixl 5 _✸_
  _✸_ : {A' A B : SubEl} {l : Carrier} → (γ : A' ⇒[ l ] A) → A ⇔[ l  ]  B → A' ⇒[ l ] B
  _✸_ {A' = A'} {A = A}  {B = B} {l = l }((A'=>'A , a'-a) , lenProof) ((A=>'B ,  a-b) §$ (B=>'A , BA≤AB) §$ AB≤A'A) = (IncUncert' (A'=>'A ∷ A=>'B) (((UNCTrans A'=>'A A=>'B ■ ( left-map (proj₂ lp) ■ R∨ AB≤A'A ))) )  , foo) , refl where
